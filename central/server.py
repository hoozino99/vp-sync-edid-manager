        ...
        for node in NODES:
            ...
            try:
                ...
                result["sync"] = {
                    ...
                    "locked": sync_data.get("sync_status") in ("OK", "ACTIVE"),
                ...
                result["disconnected_count"] = len(displays) - len(connected_displays)
...
async def api_edid_load(node_id: int, output_id: int = 0):
    node = _find_node(node_id)
    if not node:
        return {"error": "Node not found"}
    return await run_in_executor(node_remote.edid_load, node["ip"], output_id)
